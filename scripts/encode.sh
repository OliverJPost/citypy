...
encode_city() {
    ...
    citypy encode "$CITY_GPKG_FILE" --road-cluster-column "cluster_street_gmm13" --building-class-column "building_class" -o "./${CITY_GPKG_FILE%.gpkg}" >> "$CITY_LOGFILE" 2>&1
}

export -f encode_city
...
if [ -f small_files.txt ]; then
    parallel -j $SMALL_CORES --bar --progress --memfree $MEMORY_KEEPFREE --joblog "${MAIN_LOGFILE}_small.log" encode_city :::: small_files.txt
...
if [ -f medium_files.txt ]; then
    parallel -j $MEDIUM_CORES --bar --progress --memfree $MEMORY_KEEPFREE --joblog "${MAIN_LOGFILE}_medium.log" encode_city :::: medium_files.txt
...
if [ -f large_files.txt ]; then
    parallel -j $LARGE_CORES --bar --progress --memfree $MEMORY_KEEPFREE --joblog "${MAIN_LOGFILE}_large.log" encode_city :::: large_files.txt
...
if [ -f extreme_files.txt ]; then
    parallel -j $EXTREME_CORES --bar --progress --memfree $MEMORY_KEEPFREE --joblog "${MAIN_LOGFILE}_extreme.log" encode_city :::: extreme_files.txt
