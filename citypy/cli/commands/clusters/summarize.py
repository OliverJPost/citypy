...
def summarize(
    ...
    cluster_columns: List[str] = typer.Option(
        None, "--on", help="List of columns the clustering was based on."
    ),
    clusters_column: str = typer.Option(
        "cluster", "--column", "-c", help="Name of the cluster column."
    ),
    cluster_output_dir: Path = typer.Option("."),
) -> None:
    input_gpkg = input_gpkg.strip()
    gpkg_name = Path(input_gpkg).stem
    gdf = gpd.read_file(input_gpkg, layer=layer_name, engine="pyogrio")
    geometry_type = gdf.geom_type[0]
    if geometry_type == "LineString":
        gdf.set_index(["u", "v", "key"], inplace=True)
        G = graph_from_edges_only(gdf)

    grid = gpd.read_file(input_gpkg, layer="grid")

    values = grid["road_edges::c14::most_intersecting::mode_kernel3"]
    ...
    np.savez(cluster_output_dir / (gpkg_name + ".npz"), cluster_street=grid_np)

    unique_clusters = gdf[clusters_column].unique()
    for cluster in unique_clusters:
        cluster_gdf = gdf[gdf[clusters_column] == cluster]
        ...
        fp = cluster_output_dir / f"cluster_{cluster}.json"
